...
(function () {
    ...
    acss += "table#vlist td{padding:0;line-height:16px;text-align:"+docDir[0]+";white-space:nowrap;}table#vlist thead td{background-color:#FFF;height:22px;text-align:center;padding:0px 3px;}" +
        ...
        acss += "span."+allIDs[48]+" {position:absolute;margin-top:2px;"+docDir[0]+":122px !important;width:36px !important;}"+
    ...
    function neededResAdd () {	
        ...
        for( var i = 0; i < baseWrap.snapshotLength; i++ ) {
            ...
            if (base.parentNode.classList.contains("contractWrapper") || base.parentNode.classList.contains("information") || base.parentNode.classList.contains("details") || (/hero.php/.test(crtPath))) {
    ...
    function marketOffer() {
        ...
        $("button.textButtonV1.green").addClass("copy");
        // $("#build").append($("button.copy").clone().removeClass("copy").addClass("mad").attr("value","mad").html("mad"));
        // $("#build").append($("button.copy").clone().removeClass("copy").addClass("bar").attr("value","bar").html("bar"));
        // $("#build").append($("button.copy").clone().removeClass("copy").addClass("fer").attr("value","fer").html("ferr"));
        $("button.copy").clone().removeClass("copy").addClass("fer").attr("value","fer").html("ferr").insertAfter("button.copy");
        $("button.copy").clone().removeClass("copy").addClass("bar").attr("value","bar").html("bar").insertAfter("button.copy");
        $("button.copy").clone().removeClass("copy").addClass("mad").attr("value","mad").html("mad").insertAfter("button.copy");
        $("input[name='d1']").prop("checked",true);
        $("button.mad").click(function(){
            $("input[name='m2']").val($("input[name='m1']").val())
            $('select[name="rid1"]').val(4);$('select[name="rid2"]').val(1);
            $("button.copy").trigger('click');
        });
        $("button.bar").click(function(){
            $("input[name='m2']").val($("input[name='m1']").val())
            $('select[name="rid1"]').val(4);$('select[name="rid2"]').val(2);
            $("button.copy").trigger('click');
        });
        $("button.fer").click(function(){
            $("input[name='m2']").val($("input[name='m1']").val())
            $('select[name="rid1"]').val(4);$('select[name="rid2"]').val(3);
            $("button.copy").trigger('click');
        });
    ...
    function parseSpieler () {
        ...
        if( ! Rej.test(crtPath) && crtPath.indexOf('profile') != -1 ) return;
        try {
    ...
    function viewMessageIW() {
        function selectMessage (num) {
            ...
            var aLinks = $xf('.//a[(contains(@href, "berichte.php?") or contains(@href, "?id=") or contains(@href, "messages.php?")) and not(contains(@href, "toggleState=")) and not(contains(@href, "build.php"))]','f',tds[1]);
            var aLink = aLinks.href;
            var tV = /report|berichte/.test(aLink) ? 1: 0;
        ...
        for( var i = 0; i < allRows.snapshotLength; i++ ) {
            ...
            $at(newImg,[['style','position:relative;float:'+docDir[0]+';'+(/report|berichte/.test(crtPath)?'':docDir[0]+':-6px;')]]);
    ...
    function viewMessageIWK ( piBl ) {
        var iBl = piBl || cont;
        var allRows = $xf('.//td[.//a[contains(@href, "berichte.php?id=") or contains(@href, "report?id=")]]','l',iBl);
        for( var i = 0; i < allRows.snapshotLength; i++ ) {
            var td = allRows.snapshotItem(i);
            var aLink = $xf('.//a[contains(@href, "berichte") or contains(@href, "report?")]','f',td).getAttribute('href');
            var newImg = $e('IMG',[['src',img_view]]);
    ...
    function cropFind () {
        ...
        function findAnim (x) {
            ...
            if( typeof chkOasisFL[vid] != "undefined" ) {
                ...
                param = '{"x":'+x[0]+',"y":'+x[1]+'}';
                ajaxRequest(fullName+'api/v1/ajax/viewTileDetails', 'POST', param, function(ajaxResp) {
        ...
        function cropFindGetMap ( a ) {
            ...
            ajaxRequest(fullName+'api/v1/ajax/mapPositionData', 'POST', param, function(ajaxResp) {
                var mapData = eval('(' + ajaxResp.responseText + ')');
    ...
    function TM_ShowMainBuildingNumbers(){
        ...
        if ( mapInfo || mapInfo2 ) {
            countArray = 18;
            dorf = 1;
            if ( mapInfo ) { levels = $xf('./DIV','l',$g('village_map')); areaElements = $gt('area', mapInfo); } else { 
                levels = $xf('./DIV','l',mapInfo2);
                //remove duplicate nodes (travian bug)
                for( var i=1; i < levels.snapshotLength; i++ ) {
                    if (levels.snapshotItem(i).className == levels.snapshotItem(i-1).className) {
                        levels.snapshotItem(i).parentNode.removeChild(levels.snapshotItem(i));
                    }
                }
                levels = $xf('./DIV','l',mapInfo2);
            }
        ...
        for (var i = 0; i < countArray; i++) {
            if (dorf == 1) {
                if (mapInfo) {
                    ...
                    BuildingLevel = /level(\d+)/.exec(levels.snapshotItem(i).className);
                    if (! BuildingLevel) continue;
    ...
    function goldClubInfo () {
        ...
        function scanGoldRep () {
            ...
            for( var i=0; i < fList.length; i++ ) {
                ...
                var allBer = $xf('.//a[contains(@href,"berichte.php?id=") or contains(@href, "report?id=")]','l',fList[i].parentNode.parentNode.parentNode.parentNode);
    ...
    function reportsDelOrSearch () {
        ...
        var buttons = $gt('button',t.parentNode);
        ...
        for (i=0; i<buttons.length; i++) {
            ...
            if (buttons[i].value == 'archive') {
                ...
                delButt.parentNode.insertBefore(newDel, buttons[i] ? buttons[i].nextSibling : delButt.nextSibling);
            }
        ...
        if (buttons.length > 0 && delButt) {
            if ( markasreadButt ) {
                ...
                delButt.parentNode.insertBefore(newMarkAbove, $g("sAll1").parentNode.parentNode);
            }
            var newDelAbove = delButt.cloneNode(true);
            ltr ? newDelAbove.style.marginLeft = "4px" : newDelAbove.style.marginRight = "4px";
            delButt.parentNode.insertBefore(newDelAbove, $g("sAll1").parentNode.parentNode);
            if ( archiveButton ) { 
                var newArcAbove = archiveButton.cloneNode(true);
                ltr ? newArcAbove.style.marginLeft = "4px" : newArcAbove.style.marginRight = "4px";
                delButt.parentNode.insertBefore(newArcAbove, $g("sAll1").parentNode.parentNode);
            }
            var newDelAllAbove = newDel.cloneNode(true);
            delButt.parentNode.insertBefore(newDelAllAbove, $g("sAll1").parentNode.parentNode);
    ...
    function villageBMover () {
        ...
        function villBMStart () {
            if( inAct ) {
                ...
                for( var i=18; i<areas.length-3; i++) {
                    if( i<areasCount+18 ) {
        ...
        var areasCount = 20;
        for( var i=0; i<areas.length-21; i++) cross[i]=i+19;
    ...
    function displayWhatIsNew () {
        if ($g('whatsnew')) { 
            ...
            content.innerHTML = "What's new in Version "+version+" - Oct 30, 2020:<p></p><ui><li>Fixes for travian update 882 Polarbear</li></ui>";
    ...
    // start script
        ...
        if( /(?:messages.php|berichte.php|report)/.test(crtPath) ) { viewMessageIW(); }
        if( /report|berichte.php/.test(crtPath) ) reportsDelOrSearch(); else if( RB.overview[0] < -2 ) { RB.overview[0] = -1; saveCookie('OV', 'overview'); }
        ...
        if( /profile/.test(crtPath) ) {
            distanceToTargetVillages(); userActivityInfo();
            if ( ! $g('PlayerProfileEditor') ) { parseSpieler(); spielerSort(); }
        }
        if( /(report.+id=)|(berichte.php.+id=)/.test(crtPath) ) { addSpeedAndRTSend(); analyzerBattle(); getTroopNames(); }
